...
sub resolve {
    my ($self, $qname, $qtype) = (shift, shift, shift);
    my $cb = pop;
    my %opt = @_;
    ...
    $do_search = sub {
        ...
        $do_req = sub {
            ...
            my $res = $self->request($name, $qtype, $class, $timeout_at)
                or return Mojo::IOLoop->next_tick($do_search);
            ...
            while (1) {
                ...
                if (@rr) {
                    $depth--
                        or Mojo::IOLoop->next_tick($do_search); # cname chain too long
                    ...
                    # follow the cname
                    return Mojo::IOLoop->next_tick($do_req);

                } else {
                    # no, not found anything
                    return Mojo::IOLoop->next_tick($do_search);
                }
            }
        };

        Mojo::IOLoop->next_tick($do_req);
    };

    Mojo::IOLoop->next_tick($do_search);
}
...
sub parse_resolv_conf {
    ...
    for (split /\n/, $resolvconf) {
        ...
        if (/^\s*nameserver\s+(\S+)\s*$/i) {
            my $ip = $1;
            if (my $ipn = Mojo::DNS::parse_address($ip)) {
                push @{ $self->{server} }, $ip;
            } else {
...
Mojo::DNS::Resolver - a pure-perl DNS resolver

=head1 SYNOPSIS

    use Mojo::DNS::Resolver;

    my $resolver = Mojo::DNS::Resolv->new(
        timeout => 5, # global timeout variable
    );
...
This module provides a pure-perl DNS resolver used by L<Mojo::DNS>.

=head1 ATTRIBUTES

L<Mojo::DNS::Resolver> implements the following attributes.
...
if set, L<Mojo::DNS::Resolver> will cache the DNS responses internally using the supplied cache object.  The cache object should support C<get>, C<set>, and C<remove> functions (default: none)

=head2 cache_ttl

maximum ttl of the cached entries (in seconds).  Only has effect when L</"cache"> is set.

=head1 METHODS

L<Mojo::DNS::Resolver> inherits all methods from L<Mojo::Base> and implements the
following new ones.
